...
def get_image_processingResult():
    img_path = 'D:/Year4/FYP/Project/AnswerSheet_B.jpg'
    orig, ratio, screenCnt,boo = edge_detection(img_path)
    ...
    if boo==True:
        ...
        #thresh = cv2.threshold(gray, 127, 255, cv2.THRESH_TOZERO)[1]
        thresh=cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,15,10)

        cv2.imwrite('D:\Year4\FYP\Project\AnswerSheet_G.jpg', gray)
        cv2.imwrite('D:\Year4\FYP\Project\A2.jpg', thresh)
        show(thresh)

    else:
        show(orig)
    
    #thresh_resize = resize(thresh, height = 600)
    #show(thresh)
    #return thresh
